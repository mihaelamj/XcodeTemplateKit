...
private let log = Logger(subsystem: "___VARIABLE_bundleIdentifierPrefix:bundleIdentifier___.___PACKAGENAMEASRFC1034IDENTIFIER___", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<___PACKAGENAMEASIDENTIFIER___MainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			guard let audioUnit = self.audioUnit as? ___PACKAGENAMEASIDENTIFIER___AudioUnit else {
				log.error("Unable to create ___PACKAGENAMEASIDENTIFIER___AudioUnit")
				return audioUnit!
			}
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = ___PACKAGENAMEASIDENTIFIER___MainView(parameterTree: observableParameterTree)
