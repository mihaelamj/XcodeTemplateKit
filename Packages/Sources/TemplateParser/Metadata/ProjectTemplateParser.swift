...
public struct ProjectTemplateParser: Sendable {
    public init() {}

    /// Parse a TemplateInfo.plist file
    public func parse(plistPath: String) throws -> ProjectTemplateInfo {
        let url = URL(fileURLWithPath: plistPath)
        let data = try Data(contentsOf: url)

        let decoder = PropertyListDecoder()
        return try decoder.decode(ProjectTemplateInfo.self, from: data)
    }
    ...
    public func loadWithAncestors(templatePath: String, allTemplates: [String: ProjectTemplateInfo]) throws -> ResolvedProjectTemplate {
...
public struct ResolvedProjectTemplate: Sendable {
    public let info: ProjectTemplateInfo
    public let options: [TemplateOption]
    public let nodes: [String]

    public init(info: ProjectTemplateInfo, options: [TemplateOption], nodes: [String]) {
        self.info = info
