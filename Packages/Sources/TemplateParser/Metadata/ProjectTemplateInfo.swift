...
/// File or directory node within a template's file structure.
///
/// Represents a single file or directory in an Xcode template bundle,
/// forming a recursive tree structure for nested directories.
///
/// - Important: This structure is **not part of Xcode's original template format**.
///   It is generated by our extraction tool (`TemplateGenerator.swift`) and stored
///   in `_template_metadata.json` files. The original `TemplateInfo.plist` files
///   do not contain file structure information.
///
/// ## Example
/// ```swift
/// // Single file node
/// let swiftFile = FileNode(
...
/// ## Usage
/// ```swift
/// // Decode from extracted JSON metadata
/// let jsonData = try Data(contentsOf: URL(fileURLWithPath: "_template_metadata.json"))
/// let metadata = try JSONDecoder().decode(TemplateMetadata.self, from: jsonData)
///
/// // Access file structure
/// if let files = metadata.fileStructure {
///     for file in files {
///         print("\(file.name) - \(file.isDirectory ? "Directory" : "File")")
