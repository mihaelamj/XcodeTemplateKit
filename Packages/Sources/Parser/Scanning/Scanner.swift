...
extension Template.Parser.Scanning {
    ...
    public func scanAllTemplates() -> TemplateModels.Template.Model.Inventory {
        var templates: [TemplateModels.Template.Model.Metadata] = []
        ...
        return TemplateModels.Template.Model.Inventory(
    ...
    private func scanTemplates(atRoot root: String, seenPaths: inout Set<String>) -> [TemplateModels.Template.Model.Metadata] {
        var templates: [TemplateModels.Template.Model.Metadata] = []
    ...
    private func parseTemplate(at path: String, templateType: String) -> TemplateModels.Template.Model.Metadata? {
        ...
        let kind = TemplateModels.Template.Model.Kind(rawValue: kindString)
        ...
        // Extract enum fields
        let buildableType = (plist["BuildableType"] as? String).flatMap { TemplateModels.Template.Model.BuildableType(rawValue: $0) }
        let hiddenFromChooser = plist["HiddenFromChooser"] as? Bool
        let hiddenFromLibrary = plist["HiddenFromLibrary"] as? Bool
        let image = (plist["Image"] as? String).flatMap { TemplateModels.Template.Model.ImageSourceType(rawValue: $0) }
        let packageType = (plist["PackageType"] as? String).flatMap { TemplateModels.Template.Model.PackageType(rawValue: $0) }
        let project = (plist["Project"] as? String).flatMap { TemplateModels.Template.Model.ProjectConfiguration(rawValue: $0) }
        ...
        let allowedTypes = allowedTypesStrings?.map { TemplateModels.Template.Model.UniformTypeIdentifier(rawValue: $0) }
        let nodes = plist["Nodes"] as? [String]
        let platformsStrings = plist["Platforms"] as? [String]
        let platforms = platformsStrings?.map { TemplateModels.Template.Model.Platform(rawValue: $0) }
        ...
        return TemplateModels.Template.Model.Metadata(
            name: name,
            path: path,
            kind: kind,
            ancestors: ancestors?.compactMap { TemplateModels.Template.Model.Kind(rawValue: $0) },
            options: options,
    ...
    private func parseComponents(_ value: Any?) -> TemplateModels.Template.Model.Components? {
        ...
        guard let data = try? PropertyListSerialization.data(fromPropertyList: componentsArray, format: .binary, options: 0),
              let components = try? PropertyListDecoder().decode(TemplateModels.Template.Model.Components.self, from: data) else {
            return nil
        }
    ...
    private func parseTargets(_ value: Any?) -> TemplateModels.Template.Model.Targets? {
        ...
        guard let data = try? PropertyListSerialization.data(fromPropertyList: targetsArray, format: .binary, options: 0),
              let targets = try? PropertyListDecoder().decode(TemplateModels.Template.Model.Targets.self, from: data) else {
            return nil
        }
    ...
    private func parseOptionConstraints(_ value: Any?) -> TemplateModels.Template.Model.OptionConstraints? {
        ...
        guard let data = try? PropertyListSerialization.data(fromPropertyList: constraintsArray, format: .binary, options: 0),
              let constraints = try? PropertyListDecoder().decode(TemplateModels.Template.Model.OptionConstraints.self, from: data) else {
            return nil
        }
    ...
    private func parseDefinitions(_ value: Any?) -> TemplateModels.Template.Model.Definitions? {
        guard let definitionsDict = value as? [String: Any], !definitionsDict.isEmpty else {
            return nil
        }

        var definitions: [String: TemplateModels.Template.Model.DefinitionValue] = [:]
        ...
        for (key, value) in definitionsDict {
            ...
            if let stringValue = value as? String {
                ...
                if let data = try? PropertyListSerialization.data(fromPropertyList: dictValue, format: .binary, options: 0),
                   let fileDefinition = try? PropertyListDecoder().decode(TemplateModels.Template.Model.FileDefinition.self, from: data) {
                    definitions[key] = .file(fileDefinition)
                }
            }
        }

        return definitions.isEmpty ? nil : TemplateModels.Template.Model.Definitions(definitions: definitions)
    }

    private func extractOptions(from plist: [String: Any]) -> [TemplateModels.Template.Model.Option] {
        var options: [TemplateModels.Template.Model.Option] = []
        ...
        for optionDict in optionsArray {
            ...
            // Parse Units
            var units: [String: TemplateModels.Template.Model.OptionUnit]?
            if let unitsDict = optionDict["Units"] as? [String: Any] {
                var parsedUnits: [String: TemplateModels.Template.Model.OptionUnit] = [:]
                for (key, value) in unitsDict {
                    if let unitDict = value as? [String: Any] {
                        ...
                        if let data = try? PropertyListSerialization.data(fromPropertyList: unitDict, format: .binary, options: 0),
                           let unit = try? PropertyListDecoder().decode(TemplateModels.Template.Model.OptionUnit.self, from: data) {
            ...
            options.append(TemplateModels.Template.Model.Option(
    ...
    private func calculateCombinations(for options: [TemplateModels.Template.Model.Option]) -> Int {
    ...
    private func buildFileStructure(at path: String, relativeTo basePath: String) -> [TemplateModels.Template.Model.FileNode] {
        var nodes: [TemplateModels.Template.Model.FileNode] = []
        ...
        for item in contents.sorted() {
            ...
            let children: [TemplateModels.Template.Model.FileNode]?
            if isDir.boolValue {
            ...
            nodes.append(TemplateModels.Template.Model.FileNode(
                name: item,
                path: relativePath,
