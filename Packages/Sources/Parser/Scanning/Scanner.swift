...
extension Template.Parser.Scanning {
    ...
    public class Scanner {
        ...
        private func parseTemplate(at path: String, templateType: String) -> Models.Template.Model.Metadata? {
            ...
            let kind = Models.Template.Model.Kind(rawValue: kindString)
            ...
            // Extract enum fields
            let buildableType = (plist["BuildableType"] as? String).flatMap { Models.Template.Model.BuildableType(rawValue: $0) }
            let hiddenFromChooser = plist["HiddenFromChooser"] as? Bool
            let hiddenFromLibrary = plist["HiddenFromLibrary"] as? Bool
            let image = (plist["Image"] as? String).flatMap { Models.Template.Model.ImageSourceType(rawValue: $0) }
            let packageType = (plist["PackageType"] as? String).flatMap { Models.Template.Model.PackageType(rawValue: $0) }
            let project = (plist["Project"] as? String).flatMap { Models.Template.Model.ProjectConfiguration(rawValue: $0) }
            ...
            let allowedTypes = allowedTypesStrings?.map { Models.Template.Model.UniformTypeIdentifier(rawValue: $0) }
            let nodes = plist["Nodes"] as? [String]
            let platformsStrings = plist["Platforms"] as? [String]
            let platforms = platformsStrings?.map { Models.Template.Model.Platform(rawValue: $0) }
            ...
            return Models.Template.Model.Metadata(
                name: name,
                path: path,
                kind: kind,
                ancestors: ancestors?.compactMap { Models.Template.Model.Kind(rawValue: $0) },
                options: options,
        ...
        private func parseComponents(_ value: Any?) -> Models.Template.Model.Components? {
            ...
            guard let data = try? PropertyListSerialization.data(fromPropertyList: componentsArray, format: .binary, options: 0),
                  let components = try? PropertyListDecoder().decode(Models.Template.Model.Components.self, from: data) else {
                return nil
            }
        ...
        private func parseTargets(_ value: Any?) -> Models.Template.Model.Targets? {
            ...
            guard let data = try? PropertyListSerialization.data(fromPropertyList: targetsArray, format: .binary, options: 0),
                  let targets = try? PropertyListDecoder().decode(Models.Template.Model.Targets.self, from: data) else {
                return nil
            }
        ...
        private func parseOptionConstraints(_ value: Any?) -> Models.Template.Model.OptionConstraints? {
            ...
            guard let data = try? PropertyListSerialization.data(fromPropertyList: constraintsArray, format: .binary, options: 0),
                  let constraints = try? PropertyListDecoder().decode(Models.Template.Model.OptionConstraints.self, from: data) else {
                return nil
            }
        ...
        private func parseDefinitions(_ value: Any?) -> Models.Template.Model.Definitions? {
            ...
            for (key, value) in definitionsDict {
                ...
                if let stringValue = value as? String {
                    ...
                    if let data = try? PropertyListSerialization.data(fromPropertyList: dictValue, format: .binary, options: 0),
                       let fileDefinition = try? PropertyListDecoder().decode(Models.Template.Model.FileDefinition.self, from: data) {
                        definitions[key] = .file(fileDefinition)
                    }
                }
            }

            return definitions.isEmpty ? nil : Models.Template.Model.Definitions(definitions: definitions)
        }

        private func extractOptions(from plist: [String: Any]) -> [Models.Template.Model.Option] {
            var options: [Models.Template.Model.Option] = []
            ...
            for optionDict in optionsArray {
                ...
                // Parse Units using custom parser (handles heterogeneous unit items)
                let units = Template.Parser.Content.UnitsParser().parse(optionDict["Units"])

                options.append(Models.Template.Model.Option(
        ...
        private func calculateCombinations(for options: [Models.Template.Model.Option]) -> Int {
        ...
        private func buildFileStructure(at path: String, relativeTo basePath: String) -> [Models.Template.Model.FileNode] {
            ...
            for item in contents.sorted() {
                ...
                nodes.append(Models.Template.Model.FileNode(
                    name: item,
                    path: relativePath,
