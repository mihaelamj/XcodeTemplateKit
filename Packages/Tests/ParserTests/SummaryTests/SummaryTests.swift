...
struct SummaryTests {
    ...
    func parseApp_Entity() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/App_Entity_value.plist")
    ...
    func parseApp_Enum() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/App_Enum_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseApp_Privacy() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/App_Privacy_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseApplication() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Application_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseArticle_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Article_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseAssembly_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Assembly_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseAsset_Catalog() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Asset_Catalog_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseC_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/C++_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseCLIPS_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/CLIPS_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseC_File_2() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/C_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseConfiguration_Settings_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Configuration_Settings_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseDocumentation_Catalog() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Documentation_Catalog_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseEmpty_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Empty_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseEmpty() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Empty_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseExports_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Exports_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseExtension_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Extension_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseFile_List() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/File_List_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseHeader_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Header_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseINIntent_subclass() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/INIntent_subclass_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseLink_action() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Link_action_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMain_Menu() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Main_Menu_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMarkdown_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Markdown_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMetal_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Metal_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseModule_Map() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Module_Map_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseObjective_C_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Objective-C_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsePCH_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/PCH_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseProperty_List() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Property_List_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseRTF_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/RTF_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSceneKit_Asset_Cache() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SceneKit_Asset_Cache_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSceneKit_Asset_Catalog() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SceneKit_Asset_Catalog_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSceneKit_Scene_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SceneKit_Scene_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseShell_Script() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Shell_Script_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSiriKit_Intent_Definition_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SiriKit_Intent_Definition_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSpriteKit_Action() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SpriteKit_Action_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSpriteKit_Particle_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SpriteKit_Particle_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSpriteKit_Scene() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SpriteKit_Scene_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSpriteKit_Tile_Set() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SpriteKit_Tile_Set_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseStoryboard() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Storyboard_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseStrings_File_Legacy() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Strings_File_(Legacy)_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseStringsdict_File_Legacy() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Stringsdict_File_(Legacy)_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwiftUI_View() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/SwiftUI_View_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwift_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Swift_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwift_Testing_Unit_Test() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Swift_Testing_Unit_Test_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseTest_Plan() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Test_Plan_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseTouch_Alternatives_Property_List() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Touch_Alternatives_Property_List_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseTransient_App_Entity() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Transient_App_Entity_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseTutorial_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Tutorial_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseTutorial_Table_of_Contents_File() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Tutorial_Table_of_Contents_File_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseView() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/View_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseWindow() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/Window_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseXCTest_UI_Test() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/XCTest_UI_Test_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseXCTest_Unit_Test() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/SummaryTests/SummaryTests.swift", with: "Fixtures/Summary/XCTest_Unit_Test_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
