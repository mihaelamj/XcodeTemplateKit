...
struct AncestorsTests {
    ...
    func parseApp_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/App_Base_value.plist")
    ...
    func parseApp_Extension_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/App_Extension_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseApp() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseAudio_Unit_Extension_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Audio_Unit_Extension_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseAudio_Unit_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Audio_Unit_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseBase() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseBundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseCompositor_Services() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Compositor_Services_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseDocument_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Document_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseExtensionKit_Extension_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/ExtensionKit_Extension_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseExternal_Build_System() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/External_Build_System_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseFramework_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Framework_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseFramework_Testing_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Framework_Testing_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseFramework() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Framework_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseGame() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseGeneric_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Generic_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseImmersive_Space_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Immersive_Space_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_App_Extension_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_App_Extension_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_ExtensionKit_Extension_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_ExtensionKit_Extension_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_SwiftUI_App_Testing_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_SwiftUI_App_Testing_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_SwiftUI_App_UITesting_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_SwiftUI_App_UITesting_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_SwiftUI_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_SwiftUI_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_SwiftUI_Document_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_SwiftUI_Document_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_Test_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_Test_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMultiplatform_UI_Test_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Multiplatform_UI_Test_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSafari_Extension_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Safari_Extension_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSecurity_Critical() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Security_Critical_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_Metal_4_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_Metal_4_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_Metal_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_Metal_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_RealityKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_RealityKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_Safari_Extension_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_Safari_Extension_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_Safari_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_Safari_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_SceneKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_SceneKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSharedContent_SpriteKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SharedContent_SpriteKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwiftUI_App_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SwiftUI_App_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwiftUI_App_UI_Test_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SwiftUI_App_UI_Test_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwiftUI_App_Unit_Testing_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SwiftUI_App_Unit_Testing_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseSwiftUI_Document_App_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/SwiftUI_Document_App_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseUI_Test_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/UI_Test_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseUI_Testing_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/UI_Testing_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseUnit_Testing_Bundle_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Unit_Testing_Bundle_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseUnit_Testing_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Unit_Testing_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseWidget_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/Widget_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseXPC_Service_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/XPC_Service_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_App_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_App_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_App_UI_Test_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_App_UI_Test_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_Metal_4_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_Metal_4_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_Metal_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_Metal_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_Safari_Extension_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_Safari_Extension_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_Safari_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_Safari_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_SceneKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_SceneKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseiOS_SpriteKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/iOS_SpriteKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_App_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_App_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_App_UI_Test_Bundle() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_App_UI_Test_Bundle_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_Metal_4_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_Metal_4_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_Metal_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_Metal_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_Safari_Extension_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_Safari_Extension_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_Safari_Extension() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_Safari_Extension_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_SceneKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_SceneKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsemacOS_SpriteKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/macOS_SpriteKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsetvOS_App_Base() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/tvOS_App_Base_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsetvOS_Metal_4_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/tvOS_Metal_4_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsetvOS_Metal_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/tvOS_Metal_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsetvOS_SceneKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/tvOS_SceneKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsetvOS_SpriteKit_Game() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/tvOS_SpriteKit_Game_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parsevisionOS_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "ParserTests/AncestorsTests/AncestorsTests.swift", with: "Fixtures/Ancestors/visionOS_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
