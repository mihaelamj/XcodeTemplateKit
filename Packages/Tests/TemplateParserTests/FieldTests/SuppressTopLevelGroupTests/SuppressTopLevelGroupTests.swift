...
struct SuppressTopLevelGroupTests {
    ...
    func parseCompositor_Services() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "TemplateParserTests/FieldTests/SuppressTopLevelGroupTests/SuppressTopLevelGroupTests.swift", with: "Fixtures/SuppressTopLevelGroup/Compositor_Services_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]

        // Extract field - may be nil if field not present in this template
        let value = (plist["SuppressTopLevelGroup"] as? Bool).map { TemplateBooleanFormat.fromSwiftBool($0) }
    ...
    func roundTripCompositor_Services() throws {
        ...
        let value = (originalPlist["SuppressTopLevelGroup"] as? Bool).map { TemplateBooleanFormat.fromSwiftBool($0) }
        ...
        let reparsedValue = (reparsedPlist["SuppressTopLevelGroup"] as? Bool).map { TemplateBooleanFormat.fromSwiftBool($0) }
    ...
    func parseImmersive_Space_App() throws {
        let fixturePath = #filePath.replacingOccurrences(of: "TemplateParserTests/FieldTests/SuppressTopLevelGroupTests/SuppressTopLevelGroupTests.swift", with: "Fixtures/SuppressTopLevelGroup/Immersive_Space_App_value.plist")
        let plistData = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
        let plist = try PropertyListSerialization.propertyList(from: plistData, format: nil) as! [String: Any]
    ...
    func parseMissingSuppressTopLevelGroup() throws {
        ...
        let value = (testPlist["SuppressTopLevelGroup"] as? Bool).map { TemplateBooleanFormat.fromSwiftBool($0) }
