...
### 9. Profiling Procedure

To record the signposts locally (per Apple’s logging guidance in `/Volumes/Code/DeveloperExt/cupertino_test/docs/os/documentation_os_logging.md` and `/Volumes/Code/DeveloperExt/cupertino_test/docs/xcode/documentation_xcode_reducing-your-app-s-launch-time.md`):

1. Build the Mac target in Release with `-logging-level` left at default so `pointsOfInterest` logs are emitted.
2. Launch Instruments → “Points of Interest” template, select the running app, and record while running:
   - `Expand All` / `Collapse All`
   - Deep searches that return 10+ hits.
3. In the POI lane, the `FlattenNodes` intervals show duration/count, while `FilterNodes` events show query/match metadata. Export screenshots plus timing numbers into this doc once captured.
4. Repeat with VoiceOver enabled to ensure the shared row component announces “Expand node”/“Collapse node” appropriately; document observations here.
...
### 10. Collected Metrics (Run on 2025-11-18)

Run captured in `documents/Logging.trace` using the Logging → Points of Interest template in Instruments:

- **FlattenNodes (76 intervals)**
  - Total duration: 32.77 ms
  - Min duration: 11.17 µs
  - Avg duration: 431.19 µs (~0.43 ms)
  - Std Dev: 1.72 ms
  - Max duration: 7.92 ms
  - Raw sample (first 10 intervals): 7.42 ms, 26.17 µs, 7.92 ms, 16.38 µs, 35.75 µs, 12.96 µs, 27.42 µs, 11.42 µs, 25.71 µs, 13.17 µs
- **FilterNodes / FilterNodesResult events**
  - Emitted in synchronized pairs for each search; events appear every 200–400 ms during deep search, confirming the instrumentation triggers reliably. Each event logs the query/match metadata via `Logger` and can be inspected in Instruments.

Next step: capture VoiceOver/keyboard accessibility findings and, if necessary, additional traces on larger inventories.
